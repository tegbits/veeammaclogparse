...
from constant.constansEnum import TARGET_FILE, DEFAULT_START_PATH, TARGET_DATE

def getDirectory(arr, newPath = False, time = TARGET_DATE):
    ...
    for file in os.scandir(newPath):
        if (os.path.isdir(file)):
            ...
            if (re.findall(PATTERN_DATE(time), file.name)):
...
getBackupTime = lambda: datetime.now(timezone.utc).strftime("%m/%d%/%Y %H:%M:%S UTC")